...
RUN apt-get update \
    && apt-get upgrade -y \
    && apt-get install -y build-essential curl wget git vim python3-pip libopencv-dev python3-opencv \
    && apt-get clean
...
RUN pip3 install torch==1.10.1+cu113 torchvision==0.11.2+cu113 torchaudio==0.10.1+cu113 -f https://download.pytorch.org/whl/cu113/torch_stable.html \
    && pip3 install jupyter matplotlib scikit-image torchvision tensorflow
